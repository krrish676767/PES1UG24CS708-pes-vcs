...
//   3. Fill Commit struct with tree, parent, author, timestamp, message
...
int commit_create(const char *message, ObjectID *commit_id_out) {
    ...
    // Step 2: Try to read current HEAD as parent commit
    ObjectID parent_id;
    ...
    ObjectID commit_id;
    int ret = object_write(OBJ_COMMIT, commit_data, commit_len, &commit_id);
    free(commit_data);
    if (ret != 0) {
    ...
    // Return the new commit's ID if requested
